...
import { bindAll, getNumByStr } from './utils/utils';
...
class NGRMap extends NCore {
    ...
    async init(mapConfig, baseStyle) {
        ...
        this.mapView = new mapboxgl.Map(mapConfig);
    ...
    async _generateFloors() {
        ...
        for (let i = 0; i < floors.length; i += 1) {
            if (floors[i].nameEn !== 'F0') {
                this._floors.push(floors[i]);
            } else {
                this._baseStyle.sources.outdoorSource = {
                    type: 'vector',
                    url: `${this._options.vectorServer}/${floors[i].flId}/tilejsons.json`,
                };
            }
        }
